...
function renderTable(imports) {
    ...
    const tableBody = document.getElementById('imports_load');
    tableBody.innerHTML = ""; // Xóa nội dung cũ
    currentEmployees.forEach(import_item => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td>${import_item.Date}</td>
                 <td>${import_item.UserName}</td>
                 <td>${import_item.TotalMoney}</td>
                <td>
            <a href="./import-detail/?ImportID=${import_item.ImportID}">Xem Chi Tiết</a>
